...
# ============================================================================
# FONT SIZE CONTROL - Change these values to adjust all fonts at once
# ============================================================================
FONT_BASE <- 13        # Base font size for most elements
FONT_AXIS_TITLE <- 13  # Axis titles
FONT_AXIS_TEXT <- 13 # Axis labels/tick marks
FONT_LEGEND_TITLE <- 13 # Legend titles
FONT_LEGEND_TEXT <- 13 # Legend labels
FONT_STRIP <- 13        # Facet strip text
# ============================================================================
...
obelia <- read.csv("obelia_heatmap_allOTU.csv", fileEncoding = "latin1") %>%
  ...
  select(-plankton_obelia_uncorrected,-plankton_clytia_uncorrected)
```
...
obelia_long <- obelia %>%
  ...
  mutate(
    measurement_origin = case_when(
      ...
      str_detect(species_id, "plankton") ~ "Plankton Abundance (ind./L)",
...
# Split data for the four panels
...
scale_e <- scale_fill_gradientn(
  ...
  values = scales::rescale(c(0, 0.1, 0.5, 2, 3)),
  name = "eDNA RSV log(%)",
  na.value = "grey90"
)
...
scale_p <- scale_fill_gradientn(
  colors = c("#440154", "#3b528b", "#21918c", "#5ec962", "#fde725"),
  values = scales::rescale(c(0, 10, 50, 200, 300)),
...
scale_b_a <- scale_fill_gradientn(
  colors = c("#440154", "#3b528b", "#21918c", "#5ec962", "#fde725"),
  values = scales::rescale(c(0, 1, 5, 20, 30)),
  name = "Blade Coverage\nAlaria (%)",
  na.value = "grey90"
)
...
p1 <- ggplot(d_e, aes(x = as.numeric(sample_order), 
  ...
  theme(
    ...
    axis.text.x.top     = element_text(size = FONT_AXIS_TEXT),
...
p4 <- ggplot(d_b_ala, aes(x = as.numeric(sample_order),
  ...
  theme(
    axis.text.x.bottom  = element_text(size = FONT_AXIS_TEXT, vjust = 3, hjust = 0),
...
# Stack panels with shared legends
...
d_e <- obelia_date %>%
  filter(
    measurement_origin == "Plankton eDNA (RSV)",
    species_id %in% c(
      "Bougainvillia muscus", "Clytia spp.",
      "Ectopleura larynx","Obelia spp."
    )
  )
...
d_e <- d_e %>%
  mutate(
    species_id = recode_factor(
      species_id, 
      "Bougainvillia muscus" ="Bougainvillia\nmuscus",
      "Ectopleura larynx" = "Ectopleura\nlarynx"))

d_p      <- filter(obelia_date, measurement_origin == "Plankton Abundance (ind./L)")
...
p3 <- ggplot(d_b_sacc, aes(date_idx, fct_rev(species_id), fill = measurement_value)) +
  ...
  theme(
    ...
    axis.text.y      = element_text(size = FONT_AXIS_TEXT),
    axis.title.y     = element_text(size = FONT_AXIS_TITLE,
          margin = margin(t = 0.5), vjust = 1, hjust = 1.3)
  ) +
  ylab("Hydrozoa frond\nCoverage (%)")
...
p4 <- ggplot(d_b_ala, aes(date_idx, fct_rev(species_id), fill = measurement_value)) +
  ...
  theme(
    axis.text.x  = element_text(size = FONT_AXIS_TEXT, angle = 45, hjust = 1, vjust = 1.2),
    axis.ticks.x = element_blank(),
    axis.text.y  = element_text(size = FONT_AXIS_TEXT)
  ) +
  ylab(NULL)
...
final_plot <- p1 / p2 / p3 / p4 +
  plot_layout(ncol = 1, heights = c(8, 3, 2, 2), guides = "collect") &
  theme(
    legend.position   = "right",
