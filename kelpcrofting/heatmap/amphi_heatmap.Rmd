...
kelpcrofting_taxa <- kelpcrofting_taxa %>%
  mutate(species_id = case_when(
    species_id %in% "zooplank_abundance_total_malacostraca" ~ "zooplank_abundance_total_malacostraca_amphipod",
  T ~ species_id))


find_unique_amphipod_mentions <- function(df) {
  # 1. Column names containing "amphipod"
  amphipod_cols <- names(df)[str_detect(names(df), regex("amphipod", ignore_case = TRUE))]

  # 2. Unique cell values containing "amphipod"
  amphipod_cells <- df %>%
    mutate(across(everything(), as.character)) %>%
    pivot_longer(everything(), names_to = "column", values_to = "value") %>%
    filter(str_detect(value, regex("amphipod", ignore_case = TRUE))) %>%
     filter(str_detect(value, regex("amphipod", ignore_case = TRUE))) %>%
    distinct(value) %>%
    pull(value)

  list(
    column_names = unique(amphipod_cols),
    cell_values = unique(amphipod_cells)
  )
}

# Example usage
unique_amphipod_mentions <- find_unique_amphipod_mentions(kelpcrofting_taxa)

# View results
unique_amphipod_mentions$column_names
unique_amphipod_mentions$cell_values
```
...
```{r}
amphipod_terms <- c(
  "zooplank_abundance_total_amphipoda", 
  "zooplank abundance total amphipoda", 
  "otu_total_amphipoda", 
  "total amphipoda", 
  "total_amphipoda_2", 
  "total amphipoda 2", 
...
amphipod_heatmap <- kelpcrofting_taxa %>%
  mutate(parsed_date = dmy(date)) %>%
  filter(if_any(everything(), ~ str_detect(., regex(paste(amphipod_terms, collapse = "|"), ignore_case = TRUE)))) %>%
 # filter(!(str_detect(species_id, "total") & measurement_origin != "edna")) %>%
  ...
  mutate(
    ...
    y_axis = factor(origin_label, levels = c("edna", "plankton", "blade_abundance"))
  ) %>%
  arrange(parsed_date, station_no)

# Deduplicate by averaging across sample_order × y_axis
amphipod_cleaned <- amphipod_heatmap %>%
...
amphipod_top_anno <- HeatmapAnnotation(
...
col_meta <- strsplit(colnames(amphipod_mat_scaled), "_")
...
row_groups <- case_when(
  grepl("edna", rownames(amphipod_mat_scaled), ignore.case = TRUE) ~ "OTU",
  grepl("plankton", rownames(amphipod_mat_scaled), ignore.case = TRUE) ~ "Plankton",
  grepl("blade", rownames(amphipod_mat_scaled), ignore.case = TRUE) ~ "Blade abundance",
  TRUE ~ "Other"
)
...
amphi <- Heatmap(
  amphipod_mat_scaled,
  name = "Z-score",
  height = unit(nrow(amphipod_mat_scaled) * 10, "mm"),
...
```{r}
svg("amphipod_heatmap.svg", width = 14, height = 8)
draw(m)
dev.off()
```
...
```{r}
# Assume these are already created:
# bryozoa_heatmap_obj
# hydrozoa_heatmap_obj
# amphipod_heatmap_obj

combined_map <- bryozoa_heatmap_obj +
                hydro_heatmap_obj +
                 +
                gastro_heatmap_obj
