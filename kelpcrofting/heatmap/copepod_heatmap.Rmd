...
find_unique_copepod_mentions <- function(df) {
  # 1. Column names containing "copepod"
  copepod_cols <- names(df)[str_detect(names(df), regex("copepod", ignore_case = TRUE))]

  # 2. Unique cell values containing "copepod"
  copepod_cells <- df %>%
    mutate(across(everything(), as.character)) %>%
    pivot_longer(everything(), names_to = "column", values_to = "value") %>%
    filter(str_detect(value, regex("copepod", ignore_case = TRUE))) %>%
     filter(str_detect(value, regex("copepod", ignore_case = TRUE))) %>%
    distinct(value) %>%
    pull(value)

  list(
    column_names = unique(copepod_cols),
    cell_values = unique(copepod_cells)
  )
}

# Example usage
unique_copepod_mentions <- find_unique_copepod_mentions(kelpcrofting_taxa)

# View results
unique_copepod_mentions$column_names
unique_copepod_mentions$cell_values
```
...
```{r}
copepod_terms <- c(
  "zooplank_abundance_total_copepoda", 
  "zooplank abundance total copepoda", 
  "otu_total_copepoda", 
  "total copepoda", 
  "total_copepoda_2", 
  "total copepoda 2", 
...
copepod_heatmap <- kelpcrofting_taxa %>%
  mutate(parsed_date = dmy(date)) %>%
  filter(if_any(everything(), ~ str_detect(., regex(paste(copepod_terms, collapse = "|"), ignore_case = TRUE)))) %>%
...
copepod_cleaned <- copepod_heatmap %>%
...
copepod_top_anno <- HeatmapAnnotation(
...
col_meta <- strsplit(colnames(copepod_mat_scaled), "_")
...
row_groups <- case_when(
  grepl("edna", rownames(copepod_mat_scaled), ignore.case = TRUE) ~ "OTU",
  grepl("plankton", rownames(copepod_mat_scaled), ignore.case = TRUE) ~ "Plankton",
  grepl("blade", rownames(copepod_mat_scaled), ignore.case = TRUE) ~ "Blade abundance",
  TRUE ~ "Other"
)
...
cope <- Heatmap(
  copepod_mat_scaled,
  name = "Z-score",
  height = unit(nrow(copepod_mat_scaled) * 10, "mm"),
...
```{r}
svg("copepod_heatmap.svg", width = 14, height = 8)
draw(m)
dev.off()
```
