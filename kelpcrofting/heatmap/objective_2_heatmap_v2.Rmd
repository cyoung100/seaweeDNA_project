...
kelpcrofting_otu_taxonomy <- read_csv("raw_data/kelpcroft_otu.csv", skip_empty_rows = FALSE)
...
kelpcrofting_otu_taxonomy <- tibble(
  ...
  kingdom = as.character(kelpcrofting_otu_raw[1, species_cols] %>% unlist()),
  phylum = as.character(kelpcrofting_otu_raw[2, species_cols] %>% unlist()),
  class = as.character(kelpcrofting_otu_raw[3, species_cols] %>% unlist()),
  order = as.character(kelpcrofting_otu_raw[4, species_cols] %>% unlist()),
  family = as.character(kelpcrofting_otu_raw[5, species_cols] %>% unlist()),
  genus = as.character(kelpcrofting_otu_raw[6, species_cols] %>% unlist()),
  species_name = as.character(kelpcrofting_otu_raw[7, species_cols] %>% unlist())
)

kelpcrofting_otu_taxonomy$species_final <- gsub("OTU_","",kelpcrofting_otu_taxonomy$species_name)
