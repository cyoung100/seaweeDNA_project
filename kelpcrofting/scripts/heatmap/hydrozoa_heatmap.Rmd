...
hydrozoa_heatmap <- kelpcrofting_taxa %>%
  ...
  mutate(
    ...
    y_axis = factor(paste("hydrozoa", origin_label, sep = " | "),
                    levels = c("hydrozoa | edna", "hydrozoa | plankton", "hydrozoa | blade_coverage"))
  ) %>%
...
hydro <- Heatmap(
  hydrozoa_mat_scaled,
  name = "Z-score",
  height = unit(nrow(hydrozoa_mat_scaled) * 10, "mm"),
...
```{r}
svg("hydrozoa_heatmap.svg", width = 14, height = 8)
draw(h)
dev.off()
```
